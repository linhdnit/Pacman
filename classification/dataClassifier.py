...
def enhancedFeatureExtractorDigit(datum):
    ...
    "*** YOUR CODE HERE ***"
    # neu nhieu hon 1 vung khoang trang => 1. nguoc lai =>0
...
def getWhitespaceRegionCount(datum):
    #duyet cac gia tri pixel, dem so vung khoang trang
...
def enhancedPacmanFeatures(state, action):
    ...
    closestFood = 0
    foodList = successor.getFood().asList() #Tao danh sach food
    closestGhost = 0
    ghostPosition = successor.getGhostPositions() #Tim vi tri ma

    if not foodList:
        pass
    else: # chi dinh tinh nang cho food gan nhat
    ...
    if not ghostPosition:
        pass
    else: #khoang cach toi thieu den ma
        closestGhost = min(util.manhattanDistance(i, pacmanPosition) for i in ghostPosition)
    if closestGhost:
        closestGhost = 9.0 / closestGhost
    else:
        closestGhost   
    features['closestGhost'] = closestGhost #chi dinh tinh nang cho ma gan nhat
    
    features['remainingFood'] = successor.getNumFood() #chi dinh tinh nang cho food con lai
